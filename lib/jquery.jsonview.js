...
(function(jQuery) {
  ...
  JSONFormatter = (function() {
    ...
    JSONFormatter.prototype.arrayToHTML = function(array, level) {
      ...
      if (hasContents) {
        ...
        return "[<ul class=\"array level" + level + collapsible + "\" data-shot=\"" + array.length + "\">" + output + "</ul>]";
      } else {
        return '[ ]';
      }
    };
    ...
    JSONFormatter.prototype.objectToHTML = function(object, level) {
      ...
      if (hasContents) {
        collapsible = level === 0 ? '' : ' collapsible';
        var shot = Object.keys(object).slice(0, 3).join(', ');
        return "{<ul class=\"obj level" + level + collapsible + "\" data-shot=\"" + shot + "\">" + output + "</ul>}";
      } else {
        return '{ }';
      }
    };
  ...
  Collapser = (function() {
    ...
    Collapser.collapse = function(collapser) {
      ...
      ellipsis.innerHTML = $(target).data('shot');
